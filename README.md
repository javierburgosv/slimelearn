...
SlimeLearn is an AI training framework that uses [JumpSlime!](https://fenixdev.itch.io/jump-slime) as the enviroment. This project was made
...
Download the lastest release here, extract the files and run slimelearn.exe
...
And even export it to the platform of choice yourself. Be aware that this process may need some coding in order to make everything work.

Download the source code by clicking on Code > Download Zip on the top right, or just clone it using git like this:
...
must be stablished through websockets. 

When running the slimelearn.exe file, the enviroment will start and a local server will open on the 8080 port. You can connect to it using the
languages and libraries of choice.

### **The Easy Way**
You can use the [slimelearn.py](https://github.com/javierburgosv/slimelearn.py) wrapper for Python. This makes really easy to connect, configure
and subscribe to the framework. Code examples [here](https://github.com/javierburgosv/slimelearn.py/tree/main/examples).

### **The Hard Way**
For any other language, you will need to handle the comunications with the enviroment yourself. For this you need to follow this steps:

1. Connect to the server.
2. Send the configuration you want.
3. Listen to the events.

The instructions have to be in form of strings in json format with some specific fields. You can learn how to construct this requests in the next segment bellow.

For reference you can check out the [slimelearn.py source code](https://github.com/javierburgosv/slimelearn.py/blob/main/slimelearnpy/slimelearn.py) and follow the same approach.

Requests
----------
The requests have this format:
```JSON
{
...
**Mandatory arguments:**
- Mode: This stablish when we want SlimeLearn to send the status of the enviroment. It can be one of the following:
...
**Optional arguments:**
- speed: Multiply the normal speed of the enviroment in order to make trainings faster. Increasing this number can affect the results of the jumps due to changes in the physics of the engine. Keep the value under 3.4 for better results.
...
```
When this action is sent, the player will jump inmidietly (as soon as recived). This mimics the way a normal player would play, by looking at the arrow indicator and jumping in the precise moment you see it point the direction you want to jump to.
...
### Reset Request
This action helps the agent restart the position (only the position) of the player to the starting point. The configuration, time elapsed and other systems are not influenced by this petition.
...
Responses
----------
The server responses have this format:
```JSON
{
